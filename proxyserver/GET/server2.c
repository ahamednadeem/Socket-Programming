...
void send_http_response(int socket_client, const char* response)  // sent the response to the proxy server
{
	printf("Sending response to the proxy\n");
    	send(socket_client, response, strlen(response), 0);
}

void handle_http_request(int socket_client, const char* request)  // handles the incoming request and sends back the file
{
	printf("In handle_http_request\n");
    	...
    	while(base1 != base2)
  		file_name[i++] = *base1++;
  	file_name[i] = '\0';
  	printf("File Name: %s\n", file_name);  // this is the requested filename
	...
	printf("file opening successful\n");
        ...
        size_t bytesRead;
        while ((bytesRead = fread(file_buffer, 1, sizeof(file_buffer), file)) > 0) // read the file content and store it in the file_buffer
...
int main ()
{
	...
	if((socket_server = socket(AF_INET, SOCK_STREAM, 0)) < 0)  // create server socket
	{
	...
	if((bind(socket_server,(struct sockaddr*) &server_address, sizeof(server_address))) < 0)   // binds the server socket to the address
	{
		printf("Failed to Bind...\n");
		exit(1);
	}
	
	if(listen(socket_server, 5) < 0)  // listens to incoming requests
	{
	...
	while(1)
	{
		if((socket_client = accept(socket_server, NULL, NULL)) < 0)  // accept incoming connections
		{
		...
		if(read(socket_client, request, sizeof(request)) < 0)  // receive the incoming request
		{
